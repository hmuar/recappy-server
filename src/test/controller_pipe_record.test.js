...
test('update existing note record', t => {
  ...
  return pipeRecord(appState).then(state => {
    t.ok({}.hasOwnProperty.call(state, 'recordCtx'));
    const recordCtx = state.recordCtx;
    console.log(recordCtx.createdAt.toString());
    console.log(recordCtx.updatedAt.toString());
    ...
    t.ok({}.hasOwnProperty.call(recordCtx, 'createdAt'));
    t.ok({}.hasOwnProperty.call(recordCtx, 'updatedAt'));
    t.ok(recordCtx.createdAt.getTime() !== recordCtx.updatedAt.getTime());
  });
});
